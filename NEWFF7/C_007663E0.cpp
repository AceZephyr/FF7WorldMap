...
void C_00766417(int bp08) {
	...
	C_007628B5();
	C_007616B3();
	local_2.f_00_X = local_2.f_02_Y = 0;
	switch(C_00761735_PLAYER_CURRENT_MODEL()) {//current transportation mode?
		case 5: case 0xd:
			local_2.f_04_Z = bp08?bp08:C_00761FE8()?0x64:0x320;
			C_00753D00(&local_2, C_00761EEC());
		break;
		default:
			local_2.f_04_Z = 0x12c;
			C_00753D00(&local_2, C_00761EEC() + 0x400);
	}
	C_00762E87(local_2.f_00_X, local_2.f_04_Z);
}
...
void C_00766574() {
	...
	if(C_007620B6()) {//else 00766678
		...
		C_007664C2(C_00761735_PLAYER_CURRENT_MODEL());
		if(C_007618B7()) {
			C_007611AE();
			C_0075E5C0(2);
			return;
		...
		C_0076154F();
		C_007616CB();
		switch(C_00761735_PLAYER_CURRENT_MODEL()) {
			case 3://"highwind"?
				C_00761FAA(1);
				C_0074D4F7(1);//set "highwind" state?
				C_0075BB25();
...
void C_0076667C() {
	...
	local_1 = C_00761735_PLAYER_CURRENT_MODEL();//current transportation mode?
	if(D_00E3A86C == 0) {//else 007666FB
		...
		if(C_00762136_PLAYER_GROUND_TYPE() == 0x1b)
			C_007640BC(9);
	}
}
...
int C_007666FF() {
	...
	bp_04 = C_00762136_PLAYER_GROUND_TYPE();
	if(C_00761844())
...
void C_007667B2(int bp08) {
	...
	lolo.local_6 = C_00761735_PLAYER_CURRENT_MODEL() == 3;//current transportation mode?
	if(D_00E3A874) {
		D_00E3A874 --;
		return;
	}
	if(C_0074D504() == 0 && C_0074D330() != 3) {//else 00766B4F
		...
		if(D_00E3A86C == 0 && bp08 == 1) {
			...
			//goto 00766B28
		} else if(C_0074D4B6() && (lolo.local_5 & (C_00761844()?0xf040:0x40)) == 0 && D_00E3A870 > 0 && D_00E3A870 < 0xf && (C_00761769_PLAYERMODEL_MASK(0x2000) == 0 || C_00762136_PLAYER_GROUND_TYPE() == 0x12)) {//else 0076687B
			...
			C_0074D438(1, 2);
			if(lolo.local_4.f_00_X != lolo.local_10.f_00_X || lolo.local_4.f_08_Z != lolo.local_10.f_08_Z) {//else 00766AE7
				C_00767524();
				if(lolo.local_6) {
					C_00761FAA(0);
					C_00763D6C(0);
...
//00766AE5	goto 00766B28
				...
				if(C_00761735_PLAYER_CURRENT_MODEL() == 3 || C_00761735_PLAYER_CURRENT_MODEL() == 4)//current transportation mode?
					C_00763D6C(0);
				if(lolo.local_6 || (C_00761735_PLAYER_CURRENT_MODEL() == 5 && C_00761FE8()))
					C_0074D4F7(1);//set "highwind" state?
			}
		}
